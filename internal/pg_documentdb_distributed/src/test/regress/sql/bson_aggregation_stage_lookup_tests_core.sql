...
SELECT documentdb_api_internal.create_indexes_non_concurrently('db', '{ "createIndexes": "orders", "indexes": [ { "key": { "item": 1 }, "name": "orders_item_1" } ] }', TRUE);

BEGIN;
set local enable_seqscan to off;
SELECT document FROM bson_aggregation_pipeline('db', 
    '{ "aggregate": "orders", "pipeline": [ { "$match": { "item": { "$exists": true } } }, { "$lookup": { "from": "inventory", "as": "matched_docs", "localField": "item", "foreignField": "sku" } }, { "$unwind": "$matched_docs" } ], "cursor": {} }');
