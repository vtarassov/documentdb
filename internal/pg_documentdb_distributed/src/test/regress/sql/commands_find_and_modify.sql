...
PERFORM documentdb_api.insert_one('db', 'findAndModify', FORMAT('{"_id": %s, "a": "%s", "b": "%s", "c": "%s_1"}', r, (select string_agg( substr(md5(random()::text), 1, 280), '') FROM generate_series(1, 50)), (select string_agg( substr(md5(random()::text), 1, 280), '') FROM generate_series(1, 80)),r)::documentdb_core.bson);
end loop;
end;
$$;
