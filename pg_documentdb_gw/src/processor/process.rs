...
pub async fn process_request(
    ...
    transaction::handle(request, request_context, &pg_data_client).await?;
    ...
    let result = loop {
        let response = match request.request_type() {
            RequestType::Aggregate => {
                data_management::process_aggregate(request, request_context, &pg_data_client).await
            }
            RequestType::BuildInfo => constant::process_build_info(&dynamic_config).await,
            RequestType::CollStats => {
                data_management::process_coll_stats(request, request_context, &pg_data_client).await
            }
            RequestType::ConnectionStatus => {
                users::process_connection_status(request, request_context, &pg_data_client).await
            }
            RequestType::Count => {
                data_management::process_count(request, request_context, &pg_data_client).await
            }
            RequestType::Create => {
                data_description::process_create(request, request_context, &pg_data_client).await
            }
            ...
            RequestType::Distinct => {
                data_management::process_distinct(request, request_context, &pg_data_client).await
            }
            RequestType::Drop => {
            ...
            RequestType::Explain => {
                explain::process_explain(request, None, request_context, &pg_data_client).await
            }
            RequestType::Find => {
                data_management::process_find(request, request_context, &pg_data_client).await
            }
            RequestType::FindAndModify => {
                data_management::process_find_and_modify(request, request_context, &pg_data_client)
                    .await
            }
            RequestType::GetCmdLineOpts => constant::process_get_cmd_line_opts(),
            RequestType::GetDefaultRWConcern => {
                constant::process_get_rw_concern(request, request_context)
            }
            RequestType::GetLog => constant::process_get_log(),
            RequestType::GetMore => {
                cursor::process_get_more(request, request_context, &pg_data_client).await
            }
            RequestType::Hello => {
                ismaster::process(
                    "isWritablePrimary",
            ...
            RequestType::Insert => {
                data_management::process_insert(request, request_context, &pg_data_client).await
            }
            RequestType::IsDBGrid => constant::process_is_db_grid(request_context),
            RequestType::IsMaster => {
                ismaster::process("ismaster", request, request_context, &dynamic_config).await
            }
            RequestType::ListCollections => {
                data_management::process_list_collections(request, request_context, &pg_data_client)
                    .await
            }
            RequestType::ListDatabases => {
                data_management::process_list_databases(request, request_context, &pg_data_client)
                    .await
            }
            RequestType::ListIndexes => {
                indexing::process_list_indexes(request, request_context, &pg_data_client).await
            }
            ...
            RequestType::Update => {
                data_management::process_update(request, request_context, &pg_data_client).await
            }
            RequestType::Validate => {
                data_management::process_validate(request, request_context, &pg_data_client).await
            }
            RequestType::DropIndexes => {
                indexing::process_drop_indexes(request, request_context, &pg_data_client).await
            }
            ...
            RequestType::ReIndex => {
                indexing::process_reindex(request_context, &pg_data_client).await
            }
            RequestType::CurrentOp => {
                data_management::process_current_op(request, request_context, &pg_data_client).await
            }
            RequestType::CollMod => {
                data_description::process_coll_mod(request, request_context, &pg_data_client).await
            }
            RequestType::GetParameter => {
                data_management::process_get_parameter(request, request_context, &pg_data_client)
                    .await
            }
            RequestType::KillCursors => {
                cursor::process_kill_cursors(request, request_context).await
            }
            RequestType::DbStats => {
                data_management::process_db_stats(request, request_context, &pg_data_client).await
            }
            ...
            RequestType::CommitTransaction => transaction::process_commit(request_context).await,
            RequestType::AbortTransaction => transaction::process_abort(request_context).await,
            RequestType::ListCommands => constant::list_commands(),
            RequestType::EndSessions => session::end_sessions(request, request_context).await,
            ...
            RequestType::CreateUser => {
                users::process_create_user(request, request_context, &pg_data_client).await
            }
            RequestType::DropUser => {
                users::process_drop_user(request, request_context, &pg_data_client).await
            }
            RequestType::UpdateUser => {
                users::process_update_user(request, request_context, &pg_data_client).await
            }
            RequestType::UsersInfo => {
                users::process_users_info(request, request_context, &pg_data_client).await
            }
    ...
    if request_context.connection_context.transaction.is_some() {
        match result {
            ...
            {
                transaction::process_abort(request_context).await?;
            }
            _ => {}
        }
    }
